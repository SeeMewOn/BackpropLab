...
CLEANED_DIR = "../../../data/CLEANED_DATA/literature"
...
RE_DIVIDER = re.compile(r"^[\s\t]*((\.\s){3,}$|[-–—]{3,}|[-–—*_#]|\*(\s?\*)+)[\s\t]*$")
RE_TITLE = re.compile(
...
# Косметика
RE_NON_BREAKING_SPACE = re.compile(r'[\u00a0\u2007\u202f\u20af]')
RE_SPACES = re.compile(r"[\s\t]+")
RE_SPECIAL_SYMBOLS = re.compile(r"[\\*_#„“‘’]")
RE_TRANSFER = re.compile(r"[а-яa-z]-$") # Перенос слова
RE_START_DASH = re.compile(r"^[—–\-]{1,3}") # 1-3 чёрточки в начале строки
RE_DASH = re.compile(r"(?<!^)[—–\-]{1,3}")  # 1-3 чёрточки НЕ в начале строки
RE_SEPARATE_DASH = re.compile(r"(?<!^)(?<=\s)-")    # Минус НЕ в начале строки, с пробелом перед ним
RE_EQ_DASH = re.compile(r"={2,3}")
...
def _normalize_line(line: str) -> str:
    ...
    # line = re.sub(r'[\u00a0\u2007\u202f\u20af]', ' ', line)
    line = RE_NON_BREAKING_SPACE.sub(" ", line)
    # Разбиваем все составные буквы (и латинские é, и кириллические а́)
...
def main(
    ...
    writer = ShardWriter(CLEANED_DIR, "books", max_shard_size_gb)
    total_size = sum(f.stat().st_size for f in Path(DS_DIR).iterdir() if f.is_file())
    ...
    stats = {"total_docs": 0, "written_docs": 0, "duplicates": 0, "dropped": 0}
        ...
        with open(file, "r", encoding='utf-8') as f:
            for line in f:
                ...
                if line == end_tag:
                    ...
                    # Обрабатываем накопленный документ
                    block = _process_document(current_doc_lines, not_valid_threshold, english_threshold)
                    if not block:
                        stats["dropped"] += 1
                        continue
                ...
                if progress % 50_000 == 0:
                    ...
                    print(
                        ...
                        f"written_docs: {stats['written_docs']} | "
